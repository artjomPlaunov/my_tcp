...
let checksum buf src dest = 
    ...
    if i >= total_len then acc
      ...
      let b2 =
        if i + 1 < total_len then
          if i + 1 < Bytes.length pseudo then int_of_char (Bytes.get pseudo (i + 1))
  ...
  let deserialize (buf : Bytes.t) : t =
    ...
    let flags_bits = hdr_flags land 0x3F in  (* lower 6 bits: flags *)
    ...
    let pp_flags f =
      Printf.sprintf "URG:%b ACK:%b PSH:%b RST:%b SYN:%b FIN:%b"
        f.urg f.ack f.psh f.rst f.syn f.fin
